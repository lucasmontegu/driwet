...
export function SignupScreen({
	...
	const glowStyle = useAnimatedStyle(() => ({
		...
		transform: [
			{ scale: interpolate(glowOpacity.value, [0.2, 0.6], [1, 1.1]) },
		],
	}));
		...
		<View style={[styles.container, { backgroundColor: colors.background }]}>
			...
			>
				...
				<Animated.View style={ctaAnimatedStyle}>
					<Button onPress={onCreateAccount} size="lg" className="w-full">
				...
				<Pressable onPress={onContinueAsGuest} style={styles.guestButton}>
					<Text
						style={[styles.guestButtonText, { color: colors.mutedForeground }]}
					>
				...
				>
					...
					<View style={styles.socialProofTextContainer}>
						...
						<Text
							style={[styles.proofLabel, { color: colors.mutedForeground }]}
						>
...
function FloatingBadge({
	...
	return (
		<Animated.View style={[styles.floatingBadge, style, animatedStyle]}>
			<View style={[styles.badge, { backgroundColor: color }]}>{children}</View>
		</Animated.View>
	);
}
...
function AnimatedFeatureItem({
	...
	return (
		...
		>
			<View
				style={[styles.featureIcon, { backgroundColor: colors.primary + "12" }]}
			>
